...
| Rol | Usuario | Contraseña (ejemplo) | Lo que ve |
|-----|---------|----------------------|-----------|
| Admin / Asesor | `admin` (o `MQ26_USER_ADMIN`) | `MQ26_PASSWORD` | Todo el sistema + panel Admin |
| Estudio (solo lectura) | `estudio` | `MQ26_VIEWER_PASSWORD` | Mis clientes + cartera + informes |
| Inversor | `inversor` | `MQ26_INVESTOR_PASSWORD` | Suite Mi cartera; **un solo cliente/perfil** en alcance (lista acotada en BD o `MQ26_INVESTOR_CLIENTE_IDS` en .env). Sin «Cambiar cliente» en el lateral si solo hay uno |
| Asesor | `asesor` | `MQ26_ADVISOR_PASSWORD` | Suite completa profesional |
...
### Perfil 4 (Asesor conocido)

1. Login como `asesor`
2. Mostrar el banner de presets de suite
3. Tab Cartera → posición actual del cliente
4. Tab Optimizar → comparar modelos
5. Tab Informe → generar + descargar
...
2. "Lo que ves acá es el mismo tipo de análisis que un asesor haría en horas, condensado en minutos."
