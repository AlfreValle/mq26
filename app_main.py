...
Navegación por rol via ui/navigation.py — única fuente de verdad para tabs:
  ...
  - asesor     → 6 tabs institucionales (Cartera | Universo | Optimización | Riesgo | Ejecución | Reporte)
...
def _inject_css():
    ...
    _extra, _light = build_theme_css_bundle(BASE_DIR, use_light=True)
...
# ── Vaciar cartera (siempre visible; botón habilitado solo con confirmación) ───
with st.sidebar.expander("🧹 Vaciar cartera"):
    # Determinar qué carteras hay para vaciar
    _carteras_con_datos = [
        c for c in carteras_csv
        if c and "(sin datos)" not in c and c != "-- Todas las carteras --"
    ]
    if not _carteras_con_datos:
        st.caption("No hay operaciones cargadas para ninguna cartera.")
    else:
        # Si hay exactamente una cartera (flujo inversor) mostrarla fija;
        # si hay varias (flujo asesor) permitir elegir cuál vaciar.
        if len(_carteras_con_datos) == 1:
            _cartera_a_vaciar = _carteras_con_datos[0]
            st.caption(
                f"Borra **todo** el historial de operaciones de "
                f"*{_cartera_a_vaciar.split('|')[-1].strip()}* en MQ26. "
                "No se puede deshacer."
            )
        else:
            _default_vac = (
                cartera_activa
                if cartera_activa in _carteras_con_datos
                else _carteras_con_datos[0]
            )
            _cartera_a_vaciar = st.selectbox(
                "Cartera a vaciar:",
                _carteras_con_datos,
                index=_carteras_con_datos.index(_default_vac),
                key="sb_vaciar_sel",
            )
            st.caption("Borra **todo** el historial. No se puede deshacer.")
        st.checkbox(
            "Entiendo, confirmo el borrado",
            key="sb_vaciar_chk",
        )
        if st.button(
            ...
            _ctx_sb = {"engine_data": engine_data, "cartera_activa": _cartera_a_vaciar}
            try:
                _n_sb = aplicar_vaciar_cartera(_ctx_sb, _cartera_a_vaciar)
            except RuntimeError as _e_sb:
                st.sidebar.error(str(_e_sb))
            else:
                if _n_sb == 0:
                    st.sidebar.warning("No había operaciones registradas.")
                else:
                    st.sidebar.success(f"✅ {_n_sb} operaciones eliminadas.")
                for _k_sb in (
                    "inv_diagnostico", "diagnostico_cache", "pci_resultado",
                    "inv_plata_resultado", "inv_recomendacion", "inv_mix_plan",
                    "sb_vaciar_chk",
                ):
                    st.session_state.pop(_k_sb, None)
                st.rerun()
st.sidebar.markdown("---")
