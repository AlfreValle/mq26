...
def _to_date_safe(series: pd.Series) -> pd.Series:
    ...
    `.dt.date` falla con AttributeError cuando la Series contiene un único
    valor NaT escalar (NaTType), situación que ocurre con CSVs vacíos o
    columnas sin datos. Usamos `.apply()` para evitarlo.
    """
...
def _normalize_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["FECHA_COMPRA"] = _to_date_safe(out["FECHA_COMPRA"])   # seguro vs NaT escalar
