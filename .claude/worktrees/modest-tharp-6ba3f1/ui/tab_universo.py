...
def _cached_sp500_sector_panel():
    from services.sp500_sector_panel import build_sp500_sector_panel

    return build_sp500_sector_panel()


def render_tab_universo(ctx: dict) -> None:
    with st.expander(
        "🇺🇸 S&P 500 — rendimiento y valuación por sector (referencia)",
        expanded=False,
    ):
        st.caption(
            "ETFs sectoriales SPDR vs SPY: pesos aproximados del índice, retornos YTD/QTD, "
            "beta vs SPY, P/E forward y dividend yield cuando Yahoo los publica."
        )
        try:
            res = _cached_sp500_sector_panel()
            st.caption(f"Actualizado (caché ~30 min): **{res.as_of}**")
            _df = res.table.copy()
            st.dataframe(
                _df,
                use_container_width=True,
                height=min(320, 56 + 26 * len(_df.index)),
            )
            st.caption(res.disclaimer)
            if res.errors:
                st.caption("Avisos: " + " · ".join(res.errors[:6]))
        except Exception as ex:
            log_degradacion("ui.tab_universo", "sp500_sector_panel_fallo", ex)
            st.warning(f"No se pudo armar el panel sectorial: {ex}")
    ...
    try:
        ...
        log_degradacion("ui.tab_universo", "monitor_on_usd_fallo", ex)
        st.caption(f"Monitor ON USD: {ex}")
    ...
    # ── SUB-TAB: VELAS + 3 INDICADORES (solo roles no-inversor) ────────────────
    if sub_velas is not None:
        with sub_velas:
            ...
            # Universo completo: acciones + CEDEARs del universo_df
            _universo_df = getattr(engine_data, "universo_df", None)
            if _universo_df is not None and not _universo_df.empty and "Ticker" in _universo_df.columns:
                _tickers_universo = sorted(_universo_df["Ticker"].dropna().astype(str).str.strip().str.upper().unique().tolist())
            else:
                _tickers_universo = []
            _tickers_cartera_set = set(tickers_cartera or [])
            universo_velas = (
                sorted(_tickers_cartera_set) +
                [t for t in _tickers_universo if t not in _tickers_cartera_set]
            ) or ["AAPL", "MSFT", "AMZN", "GOOGL", "MELI", "SPY", "NVDA"]
    ...
    # ── SUB-TAB: MERCADO BYMA (listas en tiempo real) ────────────────────────
    with sub_byma:
        import sys as _sys
        from pathlib import Path as _BPath
        _root = str(_BPath(__file__).resolve().parent.parent)
        if _root not in _sys.path:
            _sys.path.insert(0, _root)
        from services.byma_market_data import render_byma_mercado
        render_byma_mercado()

    # ── SUB-TAB: FCIs ARGENTINA (solo roles no-inversor) ─────────────────────
    if sub_fci is not None:
        with sub_fci:
            ...
            except Exception as _e_cafci:
                log_degradacion(
                    "ui.tab_universo",
                    "cafci_fci_scanner_fallo",
                    _e_cafci,
                    perfil=str(perfil_fci)[:32],
                )
                import sys
                from pathlib import Path as _Path
