...
def build_posiciones_broker_html(
    ...
    *,
    ...
    # Banner unitario para que NO haya duda — todas las cifras de moneda son ARS
    _unidad_pill = (
        "<span style='background:#1d4ed8;color:#fff;padding:2px 8px;"
        "border-radius:6px;font-size:0.7em;font-weight:700;letter-spacing:0.04em;'>"
        "ARS</span>"
    )
    th = (
        ...
        + "<th>Cantidad</th>"
        f"<th>Precio actual<br>{_unidad_pill}</th>"
        "<th>% tenencia</th>"
        f"<th>PPC prom.<br>{_unidad_pill}</th>"
        f"<th>Valor actual<br>{_unidad_pill}</th>"
        f"<th>Costo (inicial)<br>{_unidad_pill}</th>"
        f"<th class='mq-pos-col-pnl'>Resultado<br>{_unidad_pill}</th>"
        "<th>% resultado</th>"
        "<th>% rend. USD</th>"
    )
    ...
    for _, r in work.iterrows():
        ...
        body_parts.append(
            ...
            f"<td class='mq-pos-num'{_t(inv)}>ARS$ {html_module.escape(fmt_enteros_ar(inv))}</td>"
            f"<td class='mq-pos-num mq-pos-pnl-cell'{_t(pnl)}><span class='{pnl_cls}'>"
            f"ARS$ {html_module.escape(sign_m + fmt_enteros_ar(pnl))}</span></td>"
    ...
    ccl_usado_foot = float(ccl) if ccl else 0.0
    def _t_foot(v):
        if ccl_usado_foot > 0:
            return f" title='≈ USD {v/ccl_usado_foot:,.2f}'"
        return ""
    foot = (
        "<tfoot><tr class='mq-pos-totals'>"
        f"<td colspan='{foot_label_span}'><strong>Totales (ARS)</strong></td>"
        f"<td class='mq-pos-num'{_t_foot(total_v)}><strong>ARS$ {html_module.escape(fmt_enteros_ar(total_v))}</strong></td>"
        f"<td class='mq-pos-num'{_t_foot(total_i)}><strong>ARS$ {html_module.escape(fmt_enteros_ar(total_i))}</strong></td>"
        f"<td class='mq-pos-num mq-pos-pnl-cell'{_t_foot(total_pnl)}><strong><span class='{pnl_css_class(total_pnl)}'>"
        f"ARS$ {html_module.escape(sign_tp + fmt_enteros_ar(total_pnl))}</span></strong></td>"
