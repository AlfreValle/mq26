...
from core.byma_open_data_config import (
    ...
    byma_open_data_timeout_sec,
    byma_open_data_user_agent,
)
...
def _fila_byma_comercializable_ars(row: dict[str, Any]) -> bool:
    partes: list[str] = []
    for key in (
        "currency",
        "currencyCode",
    ...
    if not partes:
        return True
    blob = " ".join(partes)
    if re.search(r"\b(ARS|PESO|PRS)\b", blob) or "ARGENTIN" in blob:
        return True
    if re.search(r"\b(USD|US\$|U\$S|DOLAR|DÓLAR|DOLARES)\b", blob):
        return False
    return True
...
def _fetch_tipo(endpoint: str) -> list[dict[str, Any]]:
    ...
    headers = {
        ...
        "User-Agent":   byma_open_data_user_agent(),
    }
...
def fetch_universo_rv_byma() -> pd.DataFrame:
    frames: list[pd.DataFrame] = []
    for endpoint, tipo in (("equities", "ACCION_LOCAL"), ("cedears", "CEDEAR")):
        ...
        if not rows_dict:
            continue
        if endpoint == "equities":
            rows_dict = [r for r in rows_dict if _fila_byma_comercializable_ars(r)]
        if not rows_dict:
            continue
        ...
        out = pd.DataFrame(
            {
                ...
                "Nombre": df.get("description", "").astype(str).str.strip() if "description" in df.columns else "",
                "Tipo": tipo,
            }
        )
        out = out[out["Ticker"] != ""]
        ...
        frames.append(out)
    if not frames:
        return pd.DataFrame(columns=["Ticker", "Nombre", "Tipo"])
    merged = pd.concat(frames, ignore_index=True)
    return merged.drop_duplicates(subset=["Ticker"], keep="first").sort_values("Ticker").reset_index(drop=True)
