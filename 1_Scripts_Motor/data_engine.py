...
def ticker_yahoo(ticker: str, universo_df=None) -> str:
    ...
    traducciones = {
        ...
        # NOTA: TLCTO es una ON (Obligación Negociable) de Telecom, NO un CEDEAR.
        # No mapear aquí: se separa en rf_tickers y no va por yfinance.
        # "TLCTO": "TEO"  ← ELIMINADO (causaba precio erróneo $14.90 USD ADR)
...
class DataEngine:
    ...
    def cargar_transaccional(self) -> pd.DataFrame:
        ...
        if not df.empty:
            if "FECHA_COMPRA" in df.columns:
                # Usar apply en vez de .dt.date para evitar AttributeError con NaT escalar
                _parsed = pd.to_datetime(df["FECHA_COMPRA"], errors="coerce")
                df["FECHA_COMPRA"] = _parsed.apply(lambda x: x.date() if pd.notna(x) else None)
            ...
            return df
        # CSV existe pero está vacío (lienzo limpio): no migrar desde Excel.
        # Solo migrar si el CSV no existe en absoluto.
    ...
    def _migrar_desde_maestra(self) -> pd.DataFrame:
        ...
        _fechas_raw = df.get("FECHA_INICIAL", pd.Series([""] * len(df)))
        _fechas_parsed = pd.to_datetime(_fechas_raw, errors="coerce")
        # .dt.date falla en NaT escalar; usar apply para manejar NaT por fila
        _fechas_date = _fechas_parsed.apply(lambda x: x.date() if pd.notna(x) else None)

        trans = pd.DataFrame({
            "CARTERA":      prop + " | " + cart,
            "FECHA_COMPRA": _fechas_date,
