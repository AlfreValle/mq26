...
def test_get_main_tabs_mq26_inversor_tres_tabs():
    # Inversor tiene 3 tabs: Mi Cartera + Plan de Objetivos + Perlas
    specs = get_main_tabs("mq26", "inversor")
    assert len(specs) == 3
    assert specs[0].tab_id == "mi_cartera"
    assert specs[1].tab_id == "plan_objetivos"
    assert specs[2].tab_id == "perlas"
...
def test_get_main_tabs_app_institutional_six_no_admin():
    specs = get_main_tabs("app", "asesor")
    assert len(specs) == 6
